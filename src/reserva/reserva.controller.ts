...
export async function createReserva(req: AuthRequest, res: Response) {
  try {
    ...
    if (!payload?.mesaId || !payload?.inicio) {
      return res.status(400).json({ message: "mesaId and inicio are required" });
    }
    ...
    res.status(400).json({ message: error.message ?? "unable to create reserva" });
  }
}
...
export async function findAllReservas(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(500).json({ message: error.message ?? "unable to list reservas" });
  }
}
...
export async function findOneReserva(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(500).json({ message: error.message ?? "unable to find reserva" });
  }
}
...
export async function cancelReserva(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(500).json({ message: error.message ?? "unable to cancel reserva" });
  }
}
...
export async function finalizeReserva(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(500).json({ message: error.message ?? "unable to finalize reserva" });
  }
}
