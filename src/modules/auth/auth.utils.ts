...
export function signAccountToken(
  payload: Omit<AccountTokenPayload, "kind">,
): string {
  const expiresIn =
    payload.role === AccountRole.ADMIN ? JWT_EXP_ADMIN : JWT_EXP_CLIENT;
  return jwt.sign({ ...payload, kind: "account" }, JWT_SECRET_KEY, {
    expiresIn,
  } as SignOptions);
}
...
export function signTableToken(
  ...
  return jwt.sign({ ...payload, kind: "table-device" }, JWT_SECRET_KEY, {
    expiresIn: JWT_EXP_TABLE,
  } as SignOptions);
}
