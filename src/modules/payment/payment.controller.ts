...
export async function createPreferenceForTable(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    const result = await paymentService.createPreferenceForTable(
      mesaId,
      req.body,
    );
    return res
    ...
    return res.status(400).json({
      message: error.message ?? "unable to create payment preference",
    });
  }
}
...
export async function getOrCreatePreferenceForPendingPedido(
  ...
  try {
    ...
    return res.status(400).json({
      message: error.message ?? "unable to prepare payment preference",
    });
  }
}
...
export async function confirmPaymentForTable(req: AuthRequest, res: Response) {
  try {
    ...
    const result = await paymentService.confirmPaymentForTable(
      mesaId,
      paymentId,
    );
    return res.status(200).json({
